    ...
    fn translate_stmts(&mut self, stmts: &[Stmt]) -> Result<CompactString, TranslateError> {
        ...
        for stmt in stmts {
            match &stmt.kind {
                ...
                StmtKind::ChangePenSize { delta } => lines.push(format_compact!("self.pen_size += {}{}", self.translate_expr(delta)?.0, fmt_comment(stmt.info.comment.as_deref()))),
                StmtKind::SetPenSize { value } => lines.push(format_compact!("self.pen_size = {}{}", self.translate_expr(value)?.0, fmt_comment(stmt.info.comment.as_deref()))),
